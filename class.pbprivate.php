...
class PBPrivate {
    ...
    public static function private_shortcode( $atts , $content = null ) {
        ...
        if((isset($_POST['export_formats']) || array_key_exists( 'format', $GLOBALS['wp_query']->query_vars )) && $options["private_boxes"]){
            return('<table class="PBPrivate"><tr><td class="PBPrivate-header"><span class="hidden"><h2>Private Content</h2></span></td><td class="PBPrivate-content">'.do_shortcode($content).'</td></tr></table>');
    ...
    public static function scssOverrides($css){
        $ncss = file_get_contents(PBPrivate__PLUGIN_DIR.'export/css.css');
        $css .= str_replace("PBPrivate__PLUGIN_URL", PBPrivate__PLUGIN_URL, $ncss);
        return $css;
    }
